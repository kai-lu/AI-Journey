...
def model(s, poles, residues, d, h):
    temp = array([r/(s-p) for p, r in zip(poles, residues)])
    return sum(temp) + d + s*h
        ...
        if p.imag != 0:
            if i == 0 or cindex[i-1] != 1:
                assert cc(poles[i]) == poles[i+1], \
                    ("Complex poles must come in conjugate pairs: %s, %s" % (poles[i], poles[i+1]))
                cindex[i] = 1
            else:
                cindex[i] = 2
...
if __name__ == '__main__':
    ...
    test_d = .2
    test_h = 2e-5
    test_f = model(test_s, test_poles, test_residues, test_d, test_h)

    # Run algorithm, results hopefully match the known model parameters
    poles, residues, d, h = vectfit_auto(test_f, test_s, n_poles=5)